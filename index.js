...
lenaCircle.addEventListener("load", async (e) => {
    ...
    if(tid){
        ...
        userInput.value = '';
        const res = await fetch(`/api/getAllMessages`, options);
        ...
        const ress = await fetch(`/api/recommendMoreProducts`, options1);
...
async function sendPrompt() {
    ...
    if (thread_id === null) {
        ...
        const reply = await fetch(`/api/startThread`, options);
        ...
        userInput.value = '';
        const reply = await fetch(`/api/messageInThread`, options);
        const replyData = await reply.json();
